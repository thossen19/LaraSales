@extends('layouts.app')
@section('title')
{{ $mode === 'grn' ? 'Direct GRN Entry' : 'Purchase Order Entry' }} - Sales ERP
@endsection
@section('content')
<div class="mb-8">
    <h2 class="text-2xl font-bold text-gray-900">{{ $mode === 'grn' ? 'Direct GRN Entry' : 'Purchase Order Entry' }}</h2>
    <p class="mt-2 text-gray-600">{{ $mode === 'grn' ? 'Record a direct goods received note for a supplier.' : 'Create a new purchase order for a supplier.' }}</p>
</div>
...
<form method="POST" action="{{ $mode === 'grn' ? route('purchases.grn.direct') : route('purchases.orders.create') }}" id="po-form">
@csrf
...
<div class="bg-white shadow rounded-lg overflow-hidden mb-6">
    <div class="px-6 py-4 bg-gradient-to-r from-indigo-600 to-indigo-700">
        <h3 class="text-lg font-semibold text-white flex items-center gap-2">
            ...
            {{ $mode === 'grn' ? 'GRN Details' : 'Order Details' }}
        </h3>
    </div>
    <div class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ $mode === 'grn' ? 'Delivery Date:' : 'Order Date:' }} <span class="text-red-500">*</span></label>
                    <input type="date" name="order_date" value="{{ $cart['order_date'] }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Reference</label>
                    <input type="text" name="reference" value="{{ $cart['reference'] }}" maxlength="60" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                </div>
                @if($supplier && $supplier->curr_code && $supplier->curr_code != 'USD')
                    <div>
                        ...
                        <p class="text-sm text-gray-900 py-2 bg-gray-50 rounded-md px-3 border border-gray-200">{{ $supplier->curr_code }}</p>
                    </div>
                @endif
            </div>
            <!-- Middle Column -->
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="supp_ref" value="{{ $cart['supp_ref'] }}" maxlength="60" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                </div>
                <div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="delivery_address" rows="5" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">{{ $cart['delivery_address'] }}</textarea>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg" id="items-table">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @forelse($cart['items'] as $line_no => $line_item)
                        ...
                        @if($cart['edit_line'] === $line_no)
                            ...
                            <tr class="bg-yellow-50">
                                ...
                                <td class="px-4 py-2">
                                    <input type="text" name="item_description" value="{{ $line_item['item_description'] }}" maxlength="150" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                </td>
                                <td class="px-4 py-2">
                                    <input type="number" name="qty" value="{{ $line_item['quantity'] }}" min="1" step="1" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm text-right" required>
                                </td>
                                <td class="px-4 py-2 text-center text-sm text-gray-700">{{ $line_item['unit'] }}</td>
                                @if($mode !== 'grn')
                                <td class="px-4 py-2">
                                    <input type="date" name="req_del_date" value="{{ $line_item['req_del_date'] }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                </td>
                                @endif
                                <td class="px-4 py-2">
                                    <input type="number" name="price" value="{{ $line_item['price'] }}" min="0" step="0.01" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm text-right">
                                </td>
                        ...
                        @else
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $line_item['stock_id'] }}</td>
                                ...
                                @if($mode !== 'grn')
                                <td class="px-4 py-3 text-sm text-gray-700 text-center">{{ $line_item['req_del_date'] }}</td>
                                @endif
                                ...
                                <td class="px-4 py-3 text-center whitespace-nowrap">
                                    <button type="submit" name="Edit{{ $line_no }}" value="1" class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 text-xs font-medium rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100 transition" title="Edit">
                                        <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        Edit
                                    </button>
                                    <button type="submit" name="Delete{{ $line_no }}" value="1" class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 text-xs font-medium rounded-md text-red-700 bg-red-50 hover:bg-red-100 transition ml-1" title="Delete" onclick="return confirm('Delete this line item?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $colspan }}" class="px-4 py-8 text-center text-gray-500">No items added yet. Select an item below and click Add Item.</td>
                        </tr>
                    @endforelse
                    ...
                    <tr class="bg-gray-50/80">
                        ...
                        <td class="px-4 py-2">
                            ...
                            @endphp
                            <input type="text" name="item_description" value="{{ $desc }}" maxlength="150" placeholder="Item description" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        </td>
                        <td class="px-4 py-2">
                            <input type="number" name="qty" value="{{ $cart['qty'] ?? 1 }}" min="1" step="1" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm text-right">
                        </td>
                        ...
                        @if($mode !== 'grn')
                        <td class="px-4 py-2">
                            <input type="date" name="req_del_date" value="{{ $cart['req_del_date'] ?? date('Y-m-d', strtotime('+7 days')) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        </td>
                        @endif
                        <td class="px-4 py-2">
                            <input type="number" name="price" value="{{ $cart['price'] ?? '0.00' }}" min="0" step="0.01" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm text-right">
                        </td>
        ...
        <div class="mt-4 bg-gray-50 rounded-lg border border-gray-200 p-4">
            <div class="flex items-center justify-end gap-6">
                <div class="text-right">
                    ...
                    <span class="ml-2 text-sm font-semibold text-gray-900">{{ number_format($total, 2) }}</span>
                </div>
                <div class="text-right">
                    <span class="text-sm text-gray-500">Amount Total:</span>
                    <span class="ml-2 text-base font-bold text-indigo-700">{{ number_format($total + $tax_total, 2) }}</span>
                </div>
            </div>
        </div>
    </div>
...
<div class="bg-white shadow rounded-lg overflow-hidden mb-6">
    ...
    <div class="p-6">
        <textarea name="comments" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" placeholder="Optional notes or memo...">{{ $cart['comments'] }}</textarea>
    </div>
</div>

<!-- Action Buttons -->
<div class="flex justify-center gap-4 mt-6">
    <button type="submit" name="CancelOrder" value="1" class="px-6 py-2.5 bg-white text-gray-700 font-medium rounded-md hover:bg-gray-100 transition border border-gray-300 shadow-sm" onclick="return confirm('Cancel this {{ $mode === 'grn' ? 'GRN' : 'purchase order' }} entry?')">
    ...
    @if(!empty($cart['items']))
        <button type="submit" name="Commit" value="1" class="px-8 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white font-medium rounded-md hover:from-indigo-700 hover:to-indigo-800 transition shadow-sm">
            ...
            @if($mode === 'grn')
                Process GRN
            @else
                {{ !empty($cart['order_no']) ? 'Update Order' : 'Place Order' }}
            @endif
        </button>
    @endif
</div>
