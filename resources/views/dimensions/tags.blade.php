@extends('layouts.app')
@section('title', 'Dimension Tags - Sales ERP')
@section('content')
<div class="mb-8">
    <h2 class="text-2xl font-bold text-gray-900">Dimension Tags</h2>
</div>
...
<form method="POST" action="{{ route('dimensions.tags') }}">
@csrf
...
<div class="bg-white shadow rounded-lg overflow-hidden mb-6">
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse($tags as $t)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-center">
                        <a href="{{ route('dimensions.tags', ['toggle_inactive' => $t->id, 'show_inactive' => $show_inactive ? '1' : null]) }}" class="text-sm {{ $t->inactive ? 'text-red-600' : 'text-green-600' }} hover:underline">{{ $t->inactive ? 'Yes' : 'No' }}</a>
                    </td>
                    <td class="px-4 py-3 text-center">
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-4 py-8 text-center text-gray-500">No dimension tags defined.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
