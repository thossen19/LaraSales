                ...
                <div class="max-h-80 overflow-y-auto border border-gray-200 rounded-md">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="itemSearchResults" class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="item-row hover:bg-indigo-50 cursor-pointer transition" data-code="{{ $item->code }}" data-name="{{ $item->name ?? '' }}" data-unit="{{ $item->unit_of_measure ?? '' }}" data-price="{{ $item->cost_price ?? 0 }}" onclick="selectItem(this)">
                                <td class="px-4 py-2.5 text-sm font-medium text-gray-900">{{ $item->code }}</td>
                                <td class="px-4 py-2.5 text-sm text-gray-600">{{ $item->name ?? '' }}</td>
                                <td class="px-4 py-2.5 text-sm text-gray-600 text-center">{{ $item->unit_of_measure ?? '' }}</td>
                                <td class="px-4 py-2.5 text-sm text-gray-600 text-right">{{ number_format($item->cost_price ?? 0, 4) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
                <div class="mt-2 text-xs text-gray-500 text-right">Showing <span id="itemCount">{{ count($items) }}</span> items</div>
            </div>
        </div>
    </div>
</div>
