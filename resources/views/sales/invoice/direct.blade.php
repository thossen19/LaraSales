@extends('layouts.app')
@section('title', 'Direct Invoice Entry - Sales ERP')
@section('content')
<div class="mb-8">
...
@if($addedID)
    <div class="bg-blue-100 border border-blue-400 text-blue-700 px-4 py-3 rounded mb-4 text-center">
        Invoice # {{ $addedID }} has been entered.
    </div>
    <div class="text-center mb-4">
        <a href="{{ route('sales.invoice.direct', ['NewInvoice' => 'Yes']) }}" class="text-indigo-600 hover:text-indigo-900 underline">Enter a New Invoice</a>
    </div>
@else

<form method="POST" action="{{ route('sales.invoice.direct') }}">
@csrf
