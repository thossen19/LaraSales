@extends('layouts.app')
@section('title', 'Sales Groups - Sales ERP')
@section('content')
<div class="mb-8">
...
<form method="POST" action="{{ route('sales.setup.groups') }}">
@csrf
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse($groups as $group)
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-sm text-gray-900">{{ $group->id }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900">{{ $group->group_name }}</td>
                    <td class="px-4 py-3 text-center">
                        <a href="{{ route('sales.setup.groups', ['toggle_inactive' => $group->id, 'show_inactive' => $show_inactive ? '1' : null]) }}" class="text-sm {{ $group->status === 'inactive' ? 'text-red-600' : 'text-green-600' }} hover:underline">{{ $group->status === 'inactive' ? 'Yes' : 'No' }}</a>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <button type="submit" name="Mode" value="Edit" onclick="this.form.selected_id.value='{{ $group->id }}'" class="text-indigo-600 hover:text-indigo-900 text-sm">Edit</button>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <button type="submit" name="Mode" value="Delete" onclick="this.form.selected_id.value='{{ $group->id }}';return confirm('Are you sure you want to delete this group?')" class="text-red-600 hover:text-red-900 text-sm">Delete</button>
                    </td>
                </tr>
            @empty
                <tr>
...
<div class="bg-white shadow rounded-lg p-6">
    <h3 class="text-lg font-medium text-gray-900 mb-4 pb-2 border-b border-gray-200">
        {{ $selected_id > 0 ? 'Edit Sales Group' : 'Add New Sales Group' }}
    </h3>
    ...
    <div class="space-y-4 max-w-lg">
        @if($selected_id > 0)
            <div>
                ...
                <input type="text" value="{{ $edit_group->id ?? '' }}" class="w-full border border-gray-300 rounded-md px-3 py-2 bg-gray-50 text-gray-500" readonly>
            </div>
        @endif
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Group Name:</label>
            <input type="text" name="description" value="{{ old('description', $edit_description) }}" maxlength="100" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
    </div>
