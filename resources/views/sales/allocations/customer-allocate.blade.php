...
function calcTotals() {
    ...
    var left = {{ abs($cart['amount']) }} - total;
...
@endpush
@extends('layouts.app')
@section('title', 'Allocate Customer Payment or Credit Note')
@section('content')
<div class="mb-8">
...
<form method="POST" action="{{ route('sales.allocations.customer-allocate') }}">
@csrf
...
<div class="bg-white shadow rounded-lg overflow-hidden mb-6">
    <div class="px-6 py-4 bg-gradient-to-r from-indigo-600 to-indigo-700">
        <h3 class="text-lg font-semibold text-white">
            ...
            Allocation of {{ $systypes[$cart['trans_type']] ?? 'Transaction' }} #{{ $cart['trans_no'] }}
        </h3>
    </div>
    <div class="p-6">
        <div class="text-lg font-medium text-gray-800 mb-2">{{ $cart['customer_name'] }}</div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm mb-4">
            <div>
                <span class="text-gray-500">Date:</span>
                <span class="font-medium text-gray-800">{{ $cart['date'] ? \Carbon\Carbon::parse($cart['date'])->format('d/m/Y') : '-' }}</span>
            </div>
            <div>
                <span class="text-gray-500">Total:</span>
                <span class="font-medium text-gray-800">${{ number_format($cart['bank_amount'] ?? $cart['amount'], 2) }} {{ $cart['currency'] }}</span>
            </div>
            @if(abs($cart['bank_amount'] ?? $cart['amount']) != abs($cart['amount']))
            <div>
                <span class="text-gray-500">Amount to be settled:</span>
                <span class="font-medium text-gray-800">${{ number_format(abs($cart['amount']), 2) }} {{ $cart['currency'] }}</span>
            </div>
            @endif
        </div>
    </div>
...
<div class="bg-white shadow rounded-lg overflow-hidden mb-6">
    <div class="px-6 py-4 bg-gradient-to-r from-indigo-600 to-indigo-700">
        <h3 class="text-lg font-semibold text-white">
            ...
            Allocated amounts in {{ $cart['currency'] }}:
        </h3>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @php $k = 0; @endphp
                @foreach($cart['allocs'] as $id => $alloc)
                    @if(abs($alloc['amount']) - $alloc['amount_allocated'] > 0)
                        @php
                        ...
                        <tr class="{{ $rowClass }} hover:bg-gray-50 transition">
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{{ $systypes[$alloc['type']] ?? 'Invoice' }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-right">
                                <a href="{{ route('sales.orders.show', $alloc['type_no']) }}" class="text-blue-600 hover:text-blue-800 font-medium" target="_blank">{{ $alloc['type_no'] }}</a>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $alloc['ref'] }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{{ $alloc['date'] ? \Carbon\Carbon::parse($alloc['date'])->format('d/m/Y') : '-' }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{{ $alloc['due_date'] ? \Carbon\Carbon::parse($alloc['due_date'])->format('d/m/Y') : '-' }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-right font-medium text-gray-900">${{ number_format(abs($alloc['amount']), 2) }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-right text-gray-700">${{ number_format($alloc['amount_allocated'], 2) }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-right text-gray-700" id="unalloc_td_{{ $id }}">${{ number_format($unAllocated, 2) }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-center">
                                <input type="text" name="amount{{ $id }}" value="{{ number_format($alloc['current_allocated'], 2, '.', '') }}"
                                    class="border border-gray-300 rounded px-2 py-0.5 text-right w-24 text-sm">
                                <input type="hidden" name="un_allocated{{ $id }}" id="unalloc_{{ $id }}" value="{{ $unAllocated }}">
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-center">
                                <a href="javascript:void(0)" onclick="allocate_all({{ $id }})" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium">All</a>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-center">
                                <a href="javascript:void(0)" onclick="allocate_none({{ $id }})" class="text-gray-500 hover:text-gray-700 text-xs font-medium">None</a>
                            </td>
                        </tr>
                    @endif
                @endforeach
    ...
    <div class="p-4 border-t border-gray-200">
        <div class="w-80 ml-auto space-y-2">
            <div class="flex justify-between text-sm">
                ...
                <span class="font-bold text-gray-900" id="total_allocated">${{ number_format($totalAllocated, 2) }}</span>
            </div>
            @php $leftToAllocate = abs($cart['amount']) - $totalAllocated; @endphp
            <div class="flex justify-between text-sm">
                <span class="text-gray-500 font-medium">Left to Allocate:</span>
                <span class="font-bold {{ $leftToAllocate < -0.001 ? 'text-red-600' : 'text-gray-900' }}" id="left_to_allocate">${{ number_format($leftToAllocate, 2) }}</span>
            </div>
        </div>
    </div>
    @else
...
<input type="hidden" name="TotalNumberOfAllocs" value="{{ count($cart['allocs']) }}">
