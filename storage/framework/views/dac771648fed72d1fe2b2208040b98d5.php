<?php $__env->startSection('title', 'Enter Supplier Invoice - Sales ERP'); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <form method="POST" action="<?php echo e(route('purchases.invoices.index')); ?>" class="space-y-6">
        <?php echo csrf_field(); ?>
        <input type="hidden" name="supplier_id" value="<?php echo e($cart['supplier_id'] ?? ''); ?>">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="tran_date" value="<?php echo e($cart['tran_date'] ?? date('Y-m-d')); ?>"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Due Date</label>
                        <input type="date" name="due_date" value="<?php echo e($cart['due_date'] ?? ''); ?>"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Reference</label>
                        <input type="text" name="reference" value="<?php echo e($cart['reference'] ?? ''); ?>"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Supplier's Reference</label>
                        <input type="text" name="supp_reference" value="<?php echo e($cart['supp_reference'] ?? ''); ?>"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        ...
                        <select name="dimension_id"
                            ...
                            <?php $__currentLoopData = $dimensions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dim): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($dim->id); ?>" <?php echo e(($cart['dimension_id'] ?? 0) == $dim->id ? 'selected' : ''); ?>><?php echo e($dim->name); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                </div>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <div class="p-6">
                ...
                <?php if(count($outstanding_grn_items) > 0): ?>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <?php $__currentLoopData = $outstanding_grn_items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php $id = $item['id']; ?>
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="px-4 py-3 text-sm font-medium text-gray-900"><?php echo e($item['item']['code'] ?? ''); ?></td>
                                        <td class="px-4 py-3 text-sm text-gray-700"><?php echo e($item['description'] ?? ''); ?></td>
                                        <td class="px-4 py-3 text-sm text-right text-gray-700"><?php echo e($item['received_quantity'] ?? 0); ?></td>
                                        <td class="px-4 py-3 text-sm text-right text-gray-700"><?php echo e($item['invoiced_quantity'] ?? 0); ?></td>
                                        <td class="px-4 py-3 text-sm text-right font-medium text-indigo-700"><?php echo e(max(0, ($item['received_quantity'] ?? 0) - ($item['invoiced_quantity'] ?? 0))); ?></td>
                                        <td class="px-4 py-3 text-center">
                                            <button type="submit" name="grn_item_id<?php echo e($id); ?>" value="<?php echo e($id); ?>"
        ...
        <?php if($cart['supplier_id'] && !empty($cart['grn_items'])): ?>
        <!-- Invoiced Items -->
        <div class="bg-white shadow rounded-lg overflow-hidden">
            <div class="px-6 py-4 bg-gradient-to-r from-indigo-600 to-indigo-700">
                <h3 class="text-lg font-semibold text-white flex items-center gap-2">
            ...
            <div class="p-6">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php $__currentLoopData = $cart['grn_items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $id => $grn_item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3 text-sm font-medium text-gray-900"><?php echo e($grn_item['stock_id']); ?></td>
                                <td class="px-4 py-3 text-sm text-gray-700"><?php echo e($grn_item['item_description']); ?></td>
                                <td class="px-4 py-3">
                                    <input type="number" step="any" min="0"
                                           name="this_quantity_inv<?php echo e($id); ?>"
                                           value="<?php echo e($grn_item['this_quantity_inv'] ?? 0); ?>"
                                           class="w-24 text-right rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                </td>
                                <td class="px-4 py-3">
                                    <input type="number" step="any" min="0"
                                           name="ChgPrice<?php echo e($id); ?>"
                                           value="<?php echo e($grn_item['chg_price'] ?? 0); ?>"
                                           class="w-28 text-right rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                </td>
                                <td class="px-4 py-3 text-sm text-right font-medium text-gray-900"><?php echo e(number_format(($grn_item['this_quantity_inv'] ?? 0) * ($grn_item['chg_price'] ?? 0), 2)); ?></td>
                                <td class="px-4 py-3 text-center">
                                    <button type="submit" name="Delete<?php echo e($id); ?>" value="<?php echo e($id); ?>"
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                    ...
                    <div>
                        ...
                        <select name="gl_dimension_id"
                            ...
                            <option value="0">None</option>
                            <?php $__currentLoopData = $dimensions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dim): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($dim->id); ?>"><?php echo e($dim->name); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                    <div>
                ...
                <div class="overflow-x-auto mt-6">
                    <table class="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php $__currentLoopData = $cart['gl_items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gl_idx => $gl_item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3 text-sm font-medium text-gray-900"><?php echo e($gl_item['gl_code']); ?></td>
                                <td class="px-4 py-3 text-sm text-right text-gray-900"><?php echo e(number_format($gl_item['amount'], 2)); ?></td>
                                <td class="px-4 py-3 text-sm text-gray-700"><?php echo e($gl_item['memo'] ?? ''); ?></td>
                                <td class="px-4 py-3 text-center">
                                    <button type="submit" name="Delete2<?php echo e($gl_idx); ?>" value="<?php echo e($gl_idx); ?>"
                                            class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 text-xs font-medium rounded-md text-red-700 bg-red-50 hover:bg-red-100 transition"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <textarea name="comments" rows="3"
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                                  placeholder="Optional notes..."><?php echo e($cart['comments'] ?? ''); ?></textarea>
                    </div>
                    <div class="bg-gray-50 rounded-lg border border-gray-200 p-4 flex flex-col justify-center">
                        <div class="flex justify-between text-sm py-1">
                            ...
                            <span class="font-semibold text-gray-900"><?php echo e(number_format($ov_amount ?? 0, 2)); ?></span>
                        </div>
                        <div class="flex justify-between text-sm font-bold border-t border-gray-200 pt-2 mt-1">
                            <span class="text-gray-800">Total Amount:</span>
                            <span class="text-indigo-700 text-base"><?php echo e(number_format($ov_amount ?? 0, 2)); ?> (<?php echo e($cart['curr_code'] ?? 'USD'); ?>)</span>
                        </div>
                    </div>
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Lupu\Desktop\laravel\fa-saas\resources\views/purchases/invoices/index.blade.php ENDPATH**/ ?>