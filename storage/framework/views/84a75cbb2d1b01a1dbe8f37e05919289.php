<?php $__env->startSection('title', 'Sales Types - Sales ERP'); ?>
...
<form method="POST" action="<?php echo e(route('sales.setup.types')); ?>">
<?php echo csrf_field(); ?>
    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            <?php $__empty_1 = true; $__currentLoopData = $types; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-gray-50 <?php echo e($type->id == $base_sales_type_id ? 'bg-yellow-50' : ''); ?>">
                    <td class="px-4 py-3 text-sm <?php echo e($type->id == $base_sales_type_id ? 'font-semibold text-gray-900' : 'text-gray-900'); ?>"><?php echo e($type->type_name); ?></td>
                    <td class="px-4 py-3 text-sm text-center <?php echo e($type->id == $base_sales_type_id ? 'text-gray-500 italic' : 'text-gray-700'); ?>">
                        <?php if($type->id == $base_sales_type_id): ?>
                            <em>Base</em>
                        <?php else: ?>
                            <?php echo e(number_format($type->factor, 4)); ?>

                        <?php endif; ?>
                    </td>
                    <td class="px-4 py-3 text-sm text-center text-gray-700"><?php echo e($type->tax_included ? 'Yes' : 'No'); ?></td>
                    <td class="px-4 py-3 text-center">
                        <a href="<?php echo e(route('sales.setup.types', ['toggle_inactive' => $type->id, 'show_inactive' => $show_inactive ? '1' : null])); ?>" class="text-sm <?php echo e($type->status === 'inactive' ? 'text-red-600' : 'text-green-600'); ?> hover:underline"><?php echo e($type->status === 'inactive' ? 'Yes' : 'No'); ?></a>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <button type="submit" name="Mode" value="Edit" onclick="this.form.selected_id.value='<?php echo e($type->id); ?>'" class="text-indigo-600 hover:text-indigo-900 text-sm">Edit</button>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <button type="submit" name="Mode" value="Delete" onclick="this.form.selected_id.value='<?php echo e($type->id); ?>';return confirm('Are you sure you want to delete this sales type?')" class="text-red-600 hover:text-red-900 text-sm">Delete</button>
                    </td>
                </tr>
...
<div class="bg-white shadow rounded-lg p-6">
    <h3 class="text-lg font-medium text-gray-900 mb-4 pb-2 border-b border-gray-200">
        <?php echo e($selected_id > 0 ? 'Edit Sales Type' : 'Add New Sales Type'); ?>
    ...
    <div class="space-y-4 max-w-lg">
        <?php if($selected_id > 0): ?>
            <div>
                ...
                <input type="text" value="<?php echo e($edit_type->id ?? ''); ?>" class="w-full border border-gray-300 rounded-md px-3 py-2 bg-gray-50 text-gray-500" readonly>
            </div>
        <?php endif; ?>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Sales Type Name:</label>
            <input type="text" name="type_name" value="<?php echo e(old('type_name', $edit_type_name)); ?>" maxlength="100" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Calculation factor:</label>
            <input type="text" name="factor" value="<?php echo e(old('factor', $edit_factor)); ?>" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
        <div>
            <label class="flex items-center text-sm text-gray-700 cursor-pointer">
                <input type="checkbox" name="tax_included" value="1" <?php echo e(old('tax_included', $edit_tax_included) ? 'checked' : ''); ?> class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Lupu\Desktop\laravel\fa-saas\resources\views/sales/setup/types.blade.php ENDPATH**/ ?>